> 源代码路径: `apps/web/src/lib/pool/blade/usePoolTransactionSourcesData.tsx`

# usePoolTransactionSourcesData Tutorial

## 1. 大白话讲讲这个hook的作用

`usePoolTransactionSourcesData` *(一个React hook，用于获取Blade池子交易来源统计数据)* 是用来获取Blade池子交易来源统计数据的hook。

简单来说：
- 获取指定时间段内池子的交易来源分布
- 展示不同来源（如不同DApp、聚合器等）的交易量占比
- 用于在UI上渲染交易来源饼图或柱状图

这是一个只读查询hook，用于展示池子的交易来源分析。
...
```typescript
{
  data: BladePoolTxSourcesChart | undefined    // 交易来源数据
  isLoading: boolean
  isError: boolean
  error: Error | null
  // ... 其他useQuery属性
...
### BladePoolTxSourcesChart类型（推断）
```typescript
interface BladePoolTxSourcesChart {
  data: {
    source: string
    volume: string
    count: number
...
2. 调用getBladePoolTxSourcesChart API
...
首先告诉AI这个hook是用来做什么的。比如这个hook是用于获取Blade池子的交易来源分布数据，支持不同时间范围的查询。
...
告诉AI这个hook需要接收什么参数，以及返回什么格式的数据。输入参数包括池子地址、链ID、时间范围（默认是一天）和启用开关。输出包含数据点数组（每个点有来源名称、交易量、交易笔数和占比）以及可选的元数据（总交易量、总笔数、开始和结束时间戳）。

**4. 实现步骤要分清楚**

AI需要知道具体的实现步骤。首先从graph-client导入获取交易来源数据的函数，然后实现 useQuery，在queryFn中调用API，配置queryKey、缓存时间和enabled条件，使用keepPreviousData来在加载新数据时保留旧数据。
...
queryKey应该包含poolAddress、chainId和duration，这样每个不同的查询都有唯一的标识。格式可以是['blade', 'pool', `${chainId}:${poolAddress}`, 'transaction-sources', duration]。
...
可以设置staleTime为60000毫秒（1分钟），gcTime为300000毫秒（5分钟）。
...
合理的staleTime和gcTime设置可以减少不必要的请求。

### 完整的生产级提示词示例

```
请帮我创建一个React Hook，用于获取Blade池子的交易来源分布数据，支持不同时间范围的查询。

需要使用@tanstack/react-query的useQuery来管理数据获取，使用@sushiswap/graph-client来获取数据。

输入参数包括池子地址、链ID、时间范围（默认DAY）和启用开关（默认true）。

输出包含数据点数组（每个点包含来源名称、交易量、交易笔数和占比）以及可选的元数据（总交易量、总笔数、时间范围）。

实现步骤：
1. 从@sushiswap/graph-client/data-api导入getBladePoolTxSourcesChart函数
2. 从@tanstack/react-query导入useQuery和keepPreviousData
3. 解构参数，设置默认值：duration = 'DAY', enabled = true
4. 实现useQuery：
   - queryKey使用['blade', 'pool', 'chainId:poolAddress', 'transaction-sources', duration]
   - queryFn调用getBladePoolTxSourcesChart({ address: poolAddress, chainId, duration })
...
```tsx
import { usePoolTransactionSourcesData } from '@sushiswap/hooks'

function TxSourcesChart({ poolAddress, chainId }) {
...
**Dos：**
- ✅ 合理设置duration的默认值
- ✅ 使用keepPreviousData避免UI闪烁
- ✅ 格式化数字和百分比展示
- ✅ 处理空数据情况

**Don'ts：**
- ❌ 不要在duration变化时立即显示loading遮盖旧数据
- ❌ 不要忽略isLoading状态
- ❌ 不要直接渲染原始数字，需要格式化
- ❌ 不要频繁切换duration
