> 源代码路径: `apps/web/src/lib/wallet/namespaces/evm/provider/use-evm-wallets.ts`
...
`EVM_WALLETS` *(一个配置文件，定义了所有支持的EVM钱包的元数据)* 配置文件定义了所有支持的钱包列表，包括：
- 钱包 ID、名称、图标
- 适配器类型（决定如何连接）
- 钱包官网 URL
...
| `adapterId` | `EvmAdapterId` *(枚举类型，定义EVM钱包的适配器类型)* | 适配器类型 |
| `isInstalled` | `boolean` | 是否已安装 |
| `isAvailable` | `boolean` | 是否可用 |
| `isRecent` | `boolean` | 是否最近使用过 |
| `uid` | `string \| undefined` | 连接器 UID（仅 injected *(指浏览器扩展类型钱包)*） |
